...
def linear_context_experiment(predictor, steps=32, batch_size=128, num_workers=1, save=True, model_name=None, csv_name='Experiments.csv', input_size=96, start_step=0, additional_steps=[], step_size=1):
    ...
    for i in steps_list:
        ...
        for split in ['test']:
            ...
            if save:
                curr_results.to_csv(f'./pretrained_experiments/Temp_res_{model_name}.csv')
            if i == 0:
                predicts['label'] = curr_true.cpu().numpy()
            predicts[str(i)]= curr_probs.cpu().numpy()
            curr_preds_df = pd.DataFrame(predicts)
            curr_preds_df.to_csv(f'./pretrained_experiments/Temp_probs_{split}_{model_name}.csv')
        print('done ', i)
