...
def add_rocauc_224(model):
    model_pth_probs = HOME_PATH + f"pretrained_experiments/{model}/Experiments_directcall_probs.csv"
    model_pth_mets = HOME_PATH + f"pretrained_experiments/{model}/Experiments_directcall_uniquedata.csv"
    ...
    roc_dict = {'Score':[],'Metric':['AUC']*len(unique_pixels),'cut_pixels':unique_pixels,'Data':['test']*len(unique_pixels)}
    for pixel in unique_pixels:
        roc_dict['Score'].append(rocauc(torch.Tensor(probs[str(pixel)]), torch.Tensor(probs['label'])).item())
    roc_dict = pd.DataFrame(roc_dict)
    model_results = pd.concat([metrics, roc_dict]).sort_values(['cut_pixels', 'Metric']).reset_index(drop=True)
...
def load_results_probs(modelname):
    ...
    if '-' not in modelname:
        input_size = 224
    pth_model_met = HOME_PATH + f"pretrained_experiments/{modelname}/Experiments_directcall_uniquedata.csv"
    pth_model_probs = HOME_PATH + f"pretrained_experiments/{modelname}/Experiments_directcall_probs.csv"
...
def plot_multiple(models, sharey=False):
    ...
    fig, ax = plt.subplots(nrows=2, ncols=2, figsize=(12, 7), sharey=sharey, sharex=True)
    for met, axis in zip(['Accuracy', 'Recall', 'Precision', 'AUC'], [ax[0][0], ax[0][1], ax[1][0], ax[1][1]]):
        print(joined_results[(joined_results.Metric==met) & (joined_results.cut_pixels==32)])
        sns.lineplot(joined_results[joined_results['Metric']==met], y='Score', x='cut_pixels', hue='Model', ax=axis)
        for transf_name in ['Swin', 'MoCo', 'supViT', 'MAE']:
            sns.scatterplot(joined_results[np.logical_and(joined_results['Model']==transf_name, joined_results['Metric']==met)], y='Score', x='cut_pixels', ax=axis)
...
def walking_change(df_melted):
    probs_melt_next = df_melted[df_melted['cut_pixels']!=32].join(df_melted[df_melted['cut_pixels']!=0].reset_index(drop=True), rsuffix='_next')
    probs_melt_next['predtype_change'] = probs_melt_next['pred_type'] + '->' + probs_melt_next['pred_type_next']
    probs_melt_next['prob_change'] = probs_melt_next['Probability'] - probs_melt_next['Probability_next']
    ...
    for i, model in enumerate(modelname):
        ...
        counts_groups_model = probs_melt_model.groupby(['pred_type', 'cut_pixels']).size().reset_index(name='counts')
        ...
        if i==0:
            sns.lineplot(probs_melt_model, x='cut_pixels', y='Probability', hue='pred_type', ax=ax[0][i])
            handles, labels = ax[0][0].get_legend_handles_labels()
            ax[i][0].get_legend().remove()
        else:
            sns.lineplot(probs_melt_model, x='cut_pixels', y='Probability', hue='pred_type', ax=ax[0][i], legend=False)
        ax[0][i].axhline(y=0.5, color='r', linestyle='--', alpha=0.5)
        sns.lineplot(counts_groups_model, x='cut_pixels', y='counts', hue='pred_type', ax=ax[1][i], legend=False)
...
def plot_walkingchange(modelname, hue='label'):
    if hue not in ['label', 'pred', 'pred_type', 'label_next','pred_next', 'pred_type_next','predtype_change']:
        return
    probs_model = load_results_probs(modelname)[1]
    probs_melt_model = melt_probs(probs_model)
    walking_change_model_melt = walking_change(probs_melt_model)
    fig, ax = plt.subplots(figsize=(5,5))
    sns.lineplot(walking_change_model_melt, x='cut_pixels', y='prob_change', hue=hue, ax=ax)
    ...
    for i, model in enumerate(modelname):
        ...
        if i==0:
            ...
            sns.lineplot(probs_melt_model, x='cut_pixels', y='Probability', hue='pred_type', ax=ax[0][i], 
                         legend=False)
        ax[0][i].axhline(y=0.5, color='r', linestyle='--', alpha=0.5)
        sns.lineplot(counts_groups_model, x='cut_pixels', y='counts', hue='pred_type', ax=ax[1][i], 
                     legend=False, marker=marker)
        ax[0][i].title.set_text(NAMEMAP[model])
        ax[1][i].set_xlabel('Context size')
        ax[i][0].set_xticks(ticks=np.arange(0,33,step), labels=np.arange(32,-1, -step))
...
def changing_probs_sample_images(probs_model_all, changes_all, model_index):
    ...
    sns.lineplot(probs_model[list(changes_sum > 16)].drop('label',axis=1).drop_duplicates().T, legend=False, ax=ax[0])  
    ...
    sns.lineplot(changing_once[changing_once.index.isin([398, 5127,24840, 1446, 21005])].T, ax=ax[1], legend=False)  
...
def plot_n_changes_context(changes_all):
    ...
    for j, model in enumerate(models):
        ...
        changes_sum_melt = pd.melt(changes[changes_sum==1].reset_index(), id_vars='index')
        changes_sum_melt_all.append(changes_sum_melt)
        number_images_change=changes_sum_melt[changes_sum_melt['value']!=0].groupby('cut_pixels').sum()['value']
        if model in ['resnet18-pcam', 'densenet121-pcam']:
            context_sizes=number_images_change.index[::3]
            context_sizes=context_sizes[:-1]+3
            number_images_change=np.add.reduceat(list(number_images_change[:-2]),
                                                          np.arange(0, len(number_images_change[:-2]), 3)).tolist()
        else:
...
def plot_histograms(walking_change_all, changes_sum_melt_all):
    for j, model in enumerate(models):
        ...
        print(model)
        walking_change_model = walking_change_model.merge(changes_sum_melt_all[j], on=['cut_pixels','index'], how='left').fillna(0)
        data = walking_change_model[walking_change_model['value'] == 1]
        fig, ax = plt.subplots(nrows=2,ncols=2,figsize=(6,6), sharex=True, sharey=False)

        bins=walking_change_model.cut_pixels_next.unique()

        for predtype, indx in zip(['FN->TP', 'TN->FP', 'FP->TN', 'TP->FN'], [(0,0),(0,1),(1,0),(1,1)]):
            sns.histplot(data[data['predtype_change']==predtype], x='cut_pixels_next', ax=ax[indx[0]][indx[1]], bins=bins)
...
def prediction_changes_per_images(changes_all):
    ...
    for j, model in enumerate(models):
        ...
        if model not in ['resnet18-pcam', 'densenet121-pcam']:
            sns.lineplot(x=changes_sum.value_counts().index*3,y=np.log(changes_sum.value_counts()), label=NAMEMAP[model],
                         marker='o')
        else:
            sns.lineplot(x=changes_sum.value_counts().index, y=np.log(changes_sum.value_counts()), label=NAMEMAP[model]) 
    plt.legend()
